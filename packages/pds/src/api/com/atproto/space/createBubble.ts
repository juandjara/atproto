...
export default function (server: Server, ctx: AppContext) {
  server.com.atproto.space.createBubble({
    ...
    handler: async ({ auth, input }) => {
      ...
      try {
        ...
        await checkPermission({
          ...
          objectType,
          objectId,
          collectionOp: LEX.ids.ComAtprotoSpaceCreateBubble,
      ...
      const bubbleRecord = await prepareCreate({
        reqDid,
        repoDid,
        space: rkey,
        collection: LEX.ids.ComAtprotoSpaceBubble,
        rkey: "self",
        record,
        validate,
      })
      ...
      const newBubbleSpiceID = `bubble:${repoDidSid}/${rkeySid}`
      const newRecordSpiceID = `record:${repoDidSid}/${rkeySid}/${atproto2spicedb(LEX.ids.ComAtprotoSpaceBubble)}/self`
      ...
      const parentRelation = await prepareCreate({
        reqDid,
        repoDid,
        space: bubbleRecord.uri.space,
        collection: LEX.ids.ComAtprotoSpaceRelation,
        rkey: ctx.genCuid['16'](),
        record: {
          resource: newBubbleSpiceID,
          relation,
          subject: parentSid,
        },
      })
      ...
      var newZookie = "???"
      const status = "???"
      const validationStatus = bubbleRecord.validationStatus
      ...
      await dualWriteTransaction({
        ...
        actorOps: async (actorTxn: ActorStoreTransactor) => {
          ...
          await actorTxn.space.insertRecord({
            uri: bubbleRecord.uri,
            parent: space,
            record: bubbleRecord.record,
            reqDid,
          })
        ...
        authzOps: async (spicedbClient) => {
          ...
          const r1 = await touchRelationship(spicedbClient, newBubbleSpiceID, relation, parentSid)
          const r2 = await touchRelationship(spicedbClient, newRecordSpiceID, relation, newBubbleSpiceID)
      ...
      // success!
      const body = {
        cid: bubbleRecord.cid.toString(),
        status: "ok",
        uri: bubbleRecord.uri.toString(),
        validationStatus,
        zookie: newZookie,
      };
