...
export default function (server: Server, ctx: AppContext) {
  server.com.atproto.space.checkPermissions({
    ...
    handler: async ({ auth, params }) => {
      ...
      try {
        ...
        // probably needs to come from resource?
        const repo = "???"
        space = "???"
        ...
        await checkPermission({
          ...
          objectType,
          objectId,
          collectionOp: LEX.ids.ComAtprotoSpaceCheckPermissions,
