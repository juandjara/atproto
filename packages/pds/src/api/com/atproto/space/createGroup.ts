...
export default function (server: Server, ctx: AppContext) {
  server.com.atproto.space.createGroup({
    ...
    handler: async ({ auth, input }) => {
      ...
      const groupRecord = await prepareCreate({
        reqDid,
        repoDid,
        space,
      ...
      const newGroupSpiceID = `group:${repoDidSid}/${spaceSid}/${atproto2spicedb(LEX.ids.ComAtprotoSpaceGroup)}/${rkeySid}`
      ...
      var newZookie = "???"
      const status = "???"
      const validationStatus = groupRecord.validationStatus
      ...
      await dualWriteTransaction({
        ...
        authzOps: async (spicedbClient) => {
          ...
          const r = await touchRelationship(spicedbClient, newGroupSpiceID, relation, parentSid)
