...
{{ $LEX_NSID := replace (.LEX.id | title)  "." "" -1 }}

// {{ .id }}


export default function (server: Server, ctx: AppContext) {
  server.com.atproto.space.{{ .NAME }}({
    ...
    {{ end }}
    handler: async ({ auth,
      {{- if .MAIN.parameters }}params,{{ end }}
      {{- if .MAIN.input }}input,{{ end }}
    }) => {
      assertSpice(ctx)
      ...
      try {
        ...
        var {
          {{ range $p, $P := .MAIN.parameters.properties -}}
          {{ $p | camel }},
          {{ end }}
        } = params
        {{ end }}
        ...
        var {
          {{ range $p, $P := .MAIN.input.schema.properties -}}
          {{ $p | camel }},
          {{ end }}
        } = input.body
        {{ end }}
        ...
        const preflight = await preflightChecks({
          ...
          repo,
          space: {{ if (or .MAIN.parameters.properties.parent .MAIN.input.schema.properties.parent) }}parent{{ else }}"<change-me>"{{ end }},
          collection: {{ if (or .MAIN.parameters.properties.collection .MAIN.input.schema.properties.collection) }}collection{{ else }}"<change-me>"{{ end }},
          rkey: {{ if (or .MAIN.parameters.properties.rkey .MAIN.input.schema.properties.rkey) }}rkey{{ else }}"<change-me>"{{ end }},
          record: {{ if (or .MAIN.parameters.properties.record .MAIN.input.schema.properties.record) }}record{{ else }}{}{{ end }},
        })
        ...
        await ctx.actorStore.transact(repoDid, async (actorTxn) => {
          const suri = AtUri.make(repoDid, "com.atproto.space.space", "self", space)
          currSpace = await actorTxn.space.getRecord(suri, null)
          spaceType = "space"
          if (currSpace === null) {
            const buri = AtUri.make(repoDid, "com.atproto.space.bubble", "self", space)
        ...
        if (currSpace === null) {
          ...
          throw new InvalidRequestError("unauthenticated, insufficient permissions, or unknown parent");
        }
        ...
        // select one of
        let objectType = "{{ .MAIN.auth.objectType }}"
        ...
        await checkPermission({
          ...
          subjectId: reqDidSid,
          permission: "{{ .MAIN.auth.permission }}",
          objectType,
          objectId,
          collectionOp: LEX.ids.{{ $LEX_NSID }},
      ...
      // success!
      const body = {
        {{ range $p, $P := .MAIN.output.schema.properties -}}
        {{$p}},
        {{ end }}
      }
