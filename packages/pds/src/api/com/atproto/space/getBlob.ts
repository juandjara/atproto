...
export default function (server: Server, ctx: AppContext) {
  server.com.atproto.space.getBlob({
    ...
    handler: async ({ auth, params, res }) => {
      ...
      try {
        ...
        await checkPermission({
          ...
          objectType,
          objectId,
          collectionOp: LEX.ids.ComAtprotoSpaceGetBlob,
      ...
      const pcid = CID.parse(cid)
      const found = await ctx.actorStore.read(repoDid, async (store) => {
        try {
          return await store.repo.blob.getBlob(pcid)
        } catch (err) {
